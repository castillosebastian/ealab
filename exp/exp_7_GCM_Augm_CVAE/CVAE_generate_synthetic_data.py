...
model = CVAE(feature_size=16063, class_size=14, latent_size=40).to(device)
...
def trainCVAE(model, optimizer, epochs, device, train_loader,num_classes=14, x_dim=16063):
    model.train()
    for epoch in range(epochs):
        ...
        try:
            for batch_idx, (x, labels) in enumerate(train_loader):                
                ...
                labels = one_hot(labels, num_classes).to(device)  # Convert labels to one-hot encoding

                optimizer.zero_grad()

                x_hat, mean, log_var = model(x, labels)  # Pass both data and labels to the model
...
for cancer_type in tqdm(unique_classes, desc="Processing Cancer Types"):
    try:
        ...
        model = CVAE(feature_size=16063, class_size=14, latent_size=40).to(device)
        trainCVAE(model, optimizer, epochs=EPOCH, device=device, train_loader=train_loader)
