...
class VAE(nn.Module):
    ...
    def __init__(self, input_dim=500, hidden_dim=400, latent_dim=200, device=device):
...
model = VAE(input_dim=500).to(device)
...
def trainVAE(model, optimizer, epochs, device, train_loader, x_dim=500):
    model.train()
...
# read from directory --------------------------------------------------------------
tra, trameta = arff.loadarff('data/madelon.trn.arff')
tst, tstmeta = arff.loadarff('data/madelon.tst.arff')
...
for dataclass in tqdm(unique_classes, desc="Processing Classes"):
    try:
        # Filter data for the current cancer type
        filtered_data = data[data[:, -1] == dataclass]
        ...
        labels_list.extend([dataclass] * num_samples)

    except Exception as e:
        print(f"Error processing {dataclass}: {e}")
