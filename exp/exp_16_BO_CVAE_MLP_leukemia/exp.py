...
# Parameters----------------------------------------------------------------------------------
exp_dir = root + "/exp/exp_16_BO_CVAE_MLP_leukemia/"
...
model = CVAE(input_size= trainloader.dataset.data.shape[1],
             labels_length=num_classes, H=best_params['hiden1'], H2=best_params['hiden2'],
...
for label in labels:
    ...
    with torch.no_grad():
        ...
        pred_with_labels = np.hstack([pred, specific_labels.numpy()[:, None]])        
        generated_data.append(pred_with_labels)

# Concatenate all generated data
generated_data = np.concatenate(generated_data, axis=0)
generated_data.shape
scaler = trainloader.dataset.standardizer
fake_data = scaler.inverse_transform(generated_data)
