...
from src.bo_vae import *
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# Parameters----------------------------------------------------------------------------------
exp_dir = root + "/exp/exp_16_BO_VAE_MLP_madelon/"
...
pred_data = pred[:, :-1]  # 2000x500
pred_class = pred[:, -1]  # 2000
scaler = trainloader.dataset.standardizer
fake_data = scaler.inverse_transform(pred_data)  
# Reshape pred_class to a 2D array
pred_class_reshaped = pred_class.reshape(-1, 1)  # 2000x1
# Correct stacking
