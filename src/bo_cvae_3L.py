...
class CVAE(nn.Module):
    def __init__(self, input_size, labels_length,  H1=100, H2=80, H3=40, latent_dim=3):
        super(CVAE, self).__init__()
        
        # Adjusted sizes to include label information
        input_size_with_label = input_size + labels_length
        adjusted_hidden_size = H3 + labels_length

        # Encoder
        self.fc1 = nn.Linear(input_size_with_label, H1)
        self.fc_bn1 = nn.BatchNorm1d(num_features=H1)
        self.fc2 = nn.Linear(H1, H2)
        self.fc_bn2 = nn.BatchNorm1d(num_features=H2)
        self.fc3 = nn.Linear(H2, H3)
        self.fc_bn3 = nn.BatchNorm1d(num_features=H3)
        
        # Latent vectors mu and sigma
        self.fc21 = nn.Linear(H3, latent_dim)
        self.fc22 = nn.Linear(H3, latent_dim)

        # Decoder
        self.fc4 = nn.Linear(latent_dim + labels_length, adjusted_hidden_size)
        self.fc_bn4 = nn.BatchNorm1d(num_features=adjusted_hidden_size)        
        self.fc5 = nn.Linear(adjusted_hidden_size, H2)
        self.fc_bn5 = nn.BatchNorm1d(num_features=H2)
        self.fc6 = nn.Linear(H2, H1)
        self.fc_bn6 = nn.BatchNorm1d(num_features=H1)
        self.fc7 = nn.Linear(H1, input_size)
    ...
    def decode(self, z, labels):
        ...
        z = self.relu(self.fc_bn5(self.fc5(z)))
        z = self.relu(self.fc_bn6(self.fc6(z)))
...
def objective(trial,trainloader,testloader, param_ranges=None, device = 'cpu', num_classes=None):
    ...
    try: 
        ...
        model = CVAE(input_size=D_in, labels_length=num_classes, H1=hiden1, H2=hiden2, H3=hiden3, latent_dim=latent_dim).float().to(device)
